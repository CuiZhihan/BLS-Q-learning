function [fgtime,MRe,C,C1,RouteAll,TimeSlot,Time,Q] = singleNodeCalMIPS(resultMode,Loc,SINR)
...
for i = 1:NN
    ...
    [MRe,Route,Q] = DQNMIPS(Ind(i),root,Layer,SINR);
...
[TimeSlot,Time] = gertTimeSlotAndTimeMIPS(RouteAll,PMat,2,SNRMat);

%% capacity
C = ((N-1)*L/sum(Time))/1000;

if resultMode == 3
    % no CoF 计算非CoF模式的容量
    [TimeSlot1,Time1] = gertTimeSlotAndTimeMIPS(RouteAll,PMat,1,SNRMat);
    C1 = ((N-1)*L/sum(Time1))/1000;
    return;    
end
