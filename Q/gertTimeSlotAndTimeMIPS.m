function [TimeSlot,Time,SINR] = gertTimeSlotAndTimeMIPS(RouteAll,PMat,maxRecNum,SINR1)
global eta B N L SNRMat
...
%         sinr = P1/(P2 + eta*B);
        for k = 1:length(ind)
            kk = ind(k);
            sender = TimeSlot{i}(kk,1);
            receiver = TimeSlot{i}(kk,2);
            sinr = SINR1(sender,receiver);
%             if sinr >= sinrtemp
%                 SINR(sender,receiver) = sinr;
%                 SINR(receiver,sender) = sinr;
%             end
        end
