...
def test():
    ...
    # # 计算分割角度和服务器放置
    # angle_per_server = 360 / server_number
    # center_x, center_y = area_length / 2, area_width / 2  # 正方形区域中心
    # #服务器位置直接导入
    # # for i in range(server_number):
    # #     server_location.append([random.uniform(0, area_length), random.uniform(0, area_width)])
    # #for i in range(server_number):
    # #    server_location.append([random.uniform(0, area_length), random.uniform(0, area_width)])
    #     # 设备处理能力 0.1-1GHz
    #     #c = random.uniform(1e7, 1e8)
    # # with (_parent / 'random_server_loc_5fixedS.csv').open(mode='r') as f:
    # #     reader = csv.reader(f, delimiter=',')  # 按行读取csv文件中的数据
    # #     # next(reader)  # 跳过表头
    # #     for row in reader:
    # #         # 将字符串转换为浮点型
    # #         row_float = [float(val) for val in row]
    # #         server_location.append(row_float)
    # server_remained_capability = [8 * 10 ** 10 for i in range(server_number)]
    # # 生成各服务器剩余算力和排队任务数据量
    # for i in range(server_number):
    #     angle = math.radians(angle_per_server * i)  # 每个扇区的起始角度
    #     radius = random.uniform(0, max_distance)  # 在0到120米之间随机选择距离
    #     x = center_x + radius * math.cos(angle)  # 计算x坐标
    #     y = center_y + radius * math.sin(angle)  # 计算y坐标
    #
    #     # 确保服务器不会放置在场地边界之外
    ...
    #     server_location.append([x, y])
    # 验证服务器之间的距离
    # for i, loc1 in enumerate(server_location):
    #     closest_distance = float('inf')  # 初始化最近距离
    #     for j, loc2 in enumerate(server_location):
    #         if i != j:  # 不与自己比较
    #             distance = math.sqrt((loc1[0] - loc2[0]) ** 2 + (loc1[1] - loc2[1]) ** 2)
    #             closest_distance = min(distance, closest_distance)
    #     assert closest_distance <= 120, f"Server {i} is too far from the closest server."

    # 传输参数
    beta_0 = -60  # dB
    bandwidth = 80 * 10 ** 6  # Hz
    sigma2 = -96  # dBm
    transmission_power = 500  # mW
    ...
    # 打印距离矩阵以检查
    #print("Distance Matrix between Devices:")
    #for row in distance_matrix_between_devices:
    #    print(row)
    # with open('device_distances.csv', 'w', newline='') as csvfile:
    #     writer = csv.writer(csvfile)
    #     # 写入矩阵的每一行
    #     for row in distance_matrix_between_devices:
    #         writer.writerow(row)

    #for row in G_matrix_between_devices:
    #    print(row)
    # with open('G_matrix_between_devices.csv', 'w', newline='') as csvfile:
    #     writer = csv.writer(csvfile)
    #     # 写入矩阵的每一行
    #     for row in G_matrix_between_devices:
    #         writer.writerow(row)
    ...
    #     writer.writerow([average_link_rate, capacity])
    #数据集写入文件
    with open(_parent / f'task_info_with_decision_{server_number}servers_{device_number}devices.csv', 'a', newline='') as csv_file:
        writer = csv.writer(csv_file)
